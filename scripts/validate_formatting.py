...
class OpenHDatasetValidator:
    ...
    REQUIRED_FEATURES = ["action.psm1.joint_position", "observation.state.psm1.joint_position"]
    ...
    def validate_directory_structure(self):
        ...
        if not data_dir.exists():
            ...
            chunk_dirs = list(data_dir.glob("case-*"))
            if not chunk_dirs:
                self.add_result(
                    ValidationLevel.ERROR,
                    category,
                    "No chunk directories found in data/ (expected format: data/case-000/)",
                )
            else:
