...
Workflow:
1. Find all dataset folders in input directory (each containing kinematic/ and time_syn/)
2. For each dataset:
   a. Detect format type (A or B) by inspecting kinematic file headers
   b. Convert all kinematic JSON files (ECM, PSM1, PSM2, PSM3)
   c. Convert all time_syn JSON files with timestamp extraction
   d. Validate converted files (optional)
3. Report conversion statistics and any errors

Input structure:
  <input_folder>/
    <dataset_id>/           # e.g., 0, 3, 5, 10, 17
      kinematic/
        ECM/                # JSON files: 0.json, 1.json, ..., n.json
        PSM1/
        PSM2/
        PSM3/               # Optional
      time_syn/             # JSON files: 0.json, 1.json, ..., n.json
      image/                # Not modified by this script
      annotation/           # Not modified by this script

Output (test_mode=true):
  <input_folder>/
    <dataset_id>/
      kinematic/
        ECM_converted/      # Converted JSON files
        PSM1_converted/
        PSM2_converted/
        PSM3_converted/
      time_syn_converted/   # Converted JSON files

Output (test_mode=false):
  Original files are replaced in-place (DESTRUCTIVE - backup recommended)

Key Features:
- Automatic format detection (Format A vs Format B)
- Intelligent timestamp backfilling for missing fields
- Robust error handling with configurable error modes
- Test mode for safe validation before in-place conversion
- Parallel processing support for large datasets (optional)
- Comprehensive validation of converted JSON files

Performance Optimizations (for large-scale data handling):
- Memory: Explicit garbage collection after each dataset, del statements for immediate cleanup
- I/O: Pre-create directories before loops to avoid repeated existence checks
- I/O: Skip reading time_syn files (not needed - all timestamps from kinematic headers)
- JSON: Efficient dict.copy() instead of deepcopy for simple timestamp dictionaries
- Iteration: Optimized file sorting with list comprehensions instead of lambda functions
- Filesystem: Breadth-first iterative search (not recursive) with configurable depth limit
- Processing: Minimal memory footprint per file, one-at-a-time processing

Format Types:
- Format A: Simple header with only sec/nsec (data/old/3, data/old/5)
- Format B: Complex header with per-topic timestamps (data/old/0, data/old/10, data/old/17)
- Target: New standardized format with list wrapper and centralized time_syn (data/new/)

See format_analysis.md for detailed format specifications.
"""
