...
The unnamed timestamp (header_measure_cp) serves as the reference baseline,
and we calculate offsets for each sensor timestamp relative to this reference.
...
class TimestampAnalyzer:
    ...
    def __init__(self, data_root: str = "../data/data_new"):
        ...
        self.base_output_dir = Path("output_cp_baseline")
        ...
        self.setpoint_measure_categories = {
            ...
            'measured': ['header_js_meas', 'header_cv', 'header_lcp'],
    ...
    def _process_json_file(self, json_file: str, camera_system: str, arm: str, dataset_name: str, subfolder: str) -> None:
        ...
        # Extract baseline timestamp (header_measure_cp - the unnamed timestamp)
        header = data.get('header', {})
        # The unnamed timestamp is directly in the header as 'sec' and 'nsec'
        if 'sec' not in header or 'nsec' not in header:
            return
            
        baseline_timestamp = header['sec'] + header['nsec'] * 1e-9
        ...
        for sensor_key, sensor_data in header.items():
            # Skip the baseline timestamp (unnamed sec/nsec)
            if sensor_key in ['sec', 'nsec']:
                continue
    ...
    def _generate_dataset_html_report(self, df: pd.DataFrame, stats: Dict, output_path: Path, dataset_name: str, subfolder: str) -> None:
        ...
        <body>
            ...
            <div class="summary">
                ...
                <p>This report analyzes timestamp offsets in synchronized multimodal data from the DVRK system using header_measure_cp as the reference baseline.</p>
