...
def _render_readme(inv: _ReleaseInventory, dataset_root: Path) -> str:
    ...
    L.append("```")
    L.append("")
    L.append("## How to pack new data into a release like this")
    L.append("")
    L.append("Step-by-step from a fresh checkout. Assumes a Linux box with conda.")
    L.append("")
    L.append("### 1. Install the toolkit")
    L.append("")
    L.append("```bash")
    L.append("git clone --recursive https://github.com/jackzhy96/dvrk_multimodal_data_collection.git")
    L.append("cd dvrk_multimodal_data_collection")
    L.append("bash env_setup/create_env.sh        # ~10–15 min")
    L.append("conda activate dvrk_multimodal_process")
    L.append("pip install -e .")
    L.append("```")
    L.append("")
    L.append("### 2. Lay out your raw clips")
    L.append("")
    L.append("The packer expects one clip per directory under "
             "`<data_dir>/<dataset_name>/<clip_idx>/`:")
    L.append("")
    L.append("```")
    L.append("<data_dir>/")
    L.append("├── online_data/")
    L.append("│   ├── 0/")
    L.append("│   │   ├── image/{left,right,side}/<i>.png")
    L.append("│   │   ├── kinematic/{ECM,PSM1,PSM2}/<i>.json")
    L.append("│   │   ├── annotation/{contact_detection,phase,step,gesture}/<i>.json")
    L.append("│   │   ├── time_syn/<i>.json")
    L.append("│   │   ├── camera_calibration/{left.yaml,right.yaml,stereo_calib_params.json}")
    L.append("│   │   ├── hand_eye_calibration/<arm>-registration-<dVRK|open-cv>.json")
    L.append("│   │   └── meta_data.json")
    L.append("│   └── 1/  2/  ...")
    L.append("└── offline_data/  ...")
    L.append("```")
    L.append("")
    L.append("If your raw clips live elsewhere (e.g. read-only archive), make symlinks:")
    L.append("")
    L.append("```bash")
    L.append("mkdir -p /tmp/pack_root")
    L.append("ln -s /path/to/archive/online_clips  /tmp/pack_root/online_data")
    L.append("ln -s /path/to/archive/offline_clips /tmp/pack_root/offline_data")
    L.append("```")
    L.append("")
    L.append("### 3. (Optional) Run preprocessing")
    L.append("")
    L.append("Only needed if you want the rectified video + depth + optical flow + "
             "kinematic heatmap MKVs in the packed release. Requires a GPU and the "
             "FoundationStereo pretrained weights (see `env_setup/INSTALL.md`).")
    L.append("")
    L.append("```bash")
    L.append("python scripts/run_all_stages.py \\")
    L.append("    path_config=jack_local_release \\")
    L.append("    clips.source=all \\")
    L.append("    path_config.data_dir=/tmp/pack_root")
    L.append("```")
    L.append("")
    L.append("Outputs land under `<data_dir>/<dataset_name>/<clip_idx>/preprocess/`. "
             "If you skip this, pass `include_preprocess=false include_video_processed=false` "
             "to `surgsync build` below.")
    L.append("")
    L.append("### 4. Pack")
    L.append("")
    L.append("```bash")
    L.append("# Whole archive, in parallel:")
    L.append("surgsync build \\")
    L.append("    clips.source=all \\")
    L.append("    path_config.data_dir=/tmp/pack_root \\")
    L.append("    path_config.dataset_root=/path/to/output/release \\")
    L.append("    parallelism=4")
    L.append("```")
    L.append("")
    L.append("Other useful invocations:")
    L.append("")
    L.append("```bash")
    L.append("# Just one clip:")
    L.append("surgsync build clips.source=list '+clips.list=[online_data/2]' \\")
    L.append("    path_config.dataset_root=/path/to/output/release")
    L.append("")
    L.append("# Skip preprocessing-dependent encoders (raw video + parquets only):")
    L.append("surgsync build clips.source=all \\")
    L.append("    path_config.dataset_root=/path/to/output/release \\")
    L.append("    include_preprocess=false include_video_processed=false \\")
    L.append("    parallelism=4")
    L.append("")
    L.append("# Force re-pack already-finalized episodes:")
    L.append("surgsync build clips.source=all force=true \\")
    L.append("    path_config.dataset_root=/path/to/output/release")
    L.append("```")
    L.append("")
    L.append("If some clips have `annotation/phase/` content that doesn't map to "
             "`workflow_description.json:_task_routing`, the packer aborts and tells "
             "you which clips to route manually. Add the overrides in a one-off "
             "config (see `config/surgsync/build_pro_ssd.yaml` for an example):")
    L.append("")
    L.append("```yaml")
    L.append("defaults:")
    L.append("  - build")
    ...
    L.append("tasks:")
    L.append("  overrides:")
    L.append('    "online_data/73": "tissue_manipulation"')
    L.append('    "online_data/74": "tissue_manipulation"')
    L.append("```")
    L.append("")
    L.append("Then invoke with `--config-name=<your file>`.")
    L.append("")
    L.append("### 5. Validate")
    L.append("")
    L.append("```bash")
    L.append("surgsync validate --layer=dataset \\")
    L.append("    --dataset-root /path/to/output/release")
    L.append("```")
    L.append("")
    L.append("Exit 0 means clean; exit 2 means at least one ERROR. Findings are "
             "informational — the packed data is on disk regardless. Document any "
             "data-quality issues in the CHANGELOG.")
    L.append("")
    L.append("### 6. Stamp release docs")
    L.append("")
    L.append("```bash")
    L.append("surgsync release /path/to/output/release \\")
    L.append("    --bump-version=minor \\")
    L.append("    --notes 'Initial pack of <description>.'")
    L.append("```")
    L.append("")
    L.append("This re-renders `README.md` and appends a new entry to `CHANGELOG.md`. "
             "`--bump-version` is `patch` / `minor` / `major` (semver); omit to "
             "keep the current `data_version`.")
    L.append("")
    L.append("### Reference docs")
    L.append("")
    L.append("- `docs/HOW_to_RUN.md` — single-page runbook (preprocessing + packing + unpacking).")
    L.append("- `docs/HOW_to_RUN_pack.md` — full `surgsync build` reference with every flag.")
    L.append("- `docs/HOW_to_RUN_unpack.md` — `surgsync unpack` reference + fidelity table.")
    L.append("- `docs/HOW_to_RUN_Process.md` — preprocessing-stage operator guide.")
    L.append("- `docs/surgsync/loader_cookbook.md` — reader API worked examples.")
    L.append("- `docs/surgsync/extension_policy.md` — how to add a new modality.")
    L.append("")
